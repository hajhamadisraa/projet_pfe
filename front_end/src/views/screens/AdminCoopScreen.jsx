...
// ─────────────────────────────────────────
// 🔧 CONFIG — Basculer ici entre mock et API
// ─────────────────────────────────────────
const USE_MOCK = false; // ← mettre false quand le backend est prêt
...
// ─────────────────────────────────────────
// 🔄 ADAPTATEUR — Format MongoDB → CoopCard
// ─────────────────────────────────────────
const adaptCoop = (c) => ({
  id:           c._id,
  nom:          c.name,
  secteur:      c.sector,
  status:       c.status === 'healthy' ? 'actif'
                : c.status === 'warning' ? 'alerte' : 'inactif',
  temp:         c.sensors?.temperature?.value
                  ? `${c.sensors.temperature.value}°C` : '--',
  humidite:     c.sensors?.humidity?.value
                  ? `${c.sensors.humidity.value}%` : '--',
  population:   c.population?.toLocaleString('fr-FR') || '0',
  updated:      c.lastSeenAt
                  ? `Vu ${new Date(c.lastSeenAt).toLocaleTimeString('fr-FR')}`
                  : 'Jamais connecté',
...
// 🧩 POPUP ALERTES
// ─────────────────────────────────────────
const AlertsPopup = ({ visible, alerts, onClose }) => (
  <Modal visible={visible} transparent animationType="fade" onRequestClose={onClose}>
    <TouchableOpacity style={s.popupOverlay} activeOpacity={1} onPress={onClose}>
      <View style={s.popup}>
        ...
        {alerts.length === 0 ? (
          ...
          <ScrollView showsVerticalScrollIndicator={false}>
            {alerts.map((a) => {
              ...
              return (
                <View key={a.id} style={s.popupAlertItem}>
...
// 🧩 COOP CARD
// ─────────────────────────────────────────
const CoopCard = ({ coop, assignedTeam, onPress, onEdit, onDelete }) => {
        ...
        <View style={s.equipChipsRow}>
          ...
          {coop.equipements.length > 4 && (
            <View style={s.equipChip}>
              <Text style={s.equipChipText}>+{coop.equipements.length - 4}</Text>
            </View>
          )}
        </View>
      )}
...
const EquipCounter = ({ equip, onIncrement, onDecrement, onRemove }) => (
  <View style={s.equipRow}>
    <View style={s.equipRowLeft}>
      <View style={s.equipIconBox}>
        <MaterialIcons name={equip.icon} size={18} color="#1B4332" />
      </View>
      <Text style={s.equipName}>{equip.label}</Text>
    </View>
    <View style={s.equipCounter}>
      <TouchableOpacity style={s.counterBtn} onPress={onDecrement} activeOpacity={0.7}>
        <MaterialIcons name="remove" size={16} color="#1B4332" />
      </TouchableOpacity>
      <Text style={s.counterValue}>{equip.qte}</Text>
      <TouchableOpacity style={s.counterBtn} onPress={onIncrement} activeOpacity={0.7}>
        <MaterialIcons name="add" size={16} color="#1B4332" />
      </TouchableOpacity>
    </View>
    <TouchableOpacity onPress={onRemove} style={s.equipRemoveBtn} activeOpacity={0.7}>
...
const CoopFormModal = ({ visible, coop, onClose, onSave, team }) => {
  ...
  React.useEffect(() => {
    if (visible) {
      setName(coop?.nom     || '');
      setSecteur(coop?.secteur || '');
      setImageUri(coop?.image  || null);
      setEquipements(coop?.equipements ? [...coop.equipements] : []);
      setCustomName('');
      setError('');
    }
  }, [visible, coop]);
  ...
  const addStandardEquip = (eq) => {
    if (equipements.find((e) => e.key === eq.key)) return;
    setEquipements((prev) => [...prev, { ...eq, qte: 1 }]);
  };
  const addCustomEquip = () => {
    if (!customName.trim()) return;
    setEquipements((prev) => [...prev, { key: `custom_${Date.now()}`, label: customName.trim(), icon: 'build', qte: 1 }]);
    setCustomName('');
  };
  ...
  const handleSave = () => {
    ...
    onSave({
      nom:          name.trim(),
      secteur:      secteur.trim() || 'Nouveau Secteur',
      status:       coop?.status || 'actif',
      image:        imageUri || null,
      equipements,
      // Champs pour l'API (utilisés par handleSave parent en mode API)
      population:   coop?.population || '0',
    });
  };
      ...
      <View style={s.modalContainer}>
        ...
        <View style={s.formSheet}>
          <View style={s.formHeader}>
            ...
            <TouchableOpacity style={s.formCloseBtn} onPress={onClose}>
          ...
          <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
            ...
            <View style={s.formSection}>
              ...
              <TextInput
                style={[s.formInput, error ? { borderColor: '#EF4444' } : {}]}
                value={name}
                onChangeText={(t) => { setName(t); setError(''); }}
                placeholder="ex: Hangar de Croissance Nord-Est"
            ...
            <View style={s.formSection}>
              ...
              <View style={s.standardEquipGrid}>
                {STANDARD_EQUIPEMENTS.map((eq) => {
                  ...
                  return (
                    <TouchableOpacity
                      key={eq.key}
                      style={[s.standardEquipBtn, isAdded && s.standardEquipBtnAdded]}
                      onPress={() => addStandardEquip(eq)}
                      activeOpacity={0.8}
                    >
              ...
              {equipements.length > 0 && (
                <View style={s.equipAddedList}>
                  ...
                  {equipements.map((eq) => (
                    <EquipCounter
                      key={eq.key} equip={eq}
                      onIncrement={() => incrementQte(eq.key)}
                      onDecrement={() => decrementQte(eq.key)}
                      onRemove={() => removeEquip(eq.key)}
                    />
                  ))}
                </View>
              )}
              ...
              <View style={s.customEquipRow}>
                ...
                />
                <TouchableOpacity style={s.customEquipAddBtn} onPress={addCustomEquip} activeOpacity={0.8}>
            ...
            <View style={s.formSection}>
              ...
              <View style={s.teamPreviewList}>
                {team.map((m) => (
                  <View key={m.id} style={s.teamPreviewItem}>
                    <Image source={{ uri: m.avatar }} style={s.teamPreviewAvatar} />
                    <View>
                      <Text style={s.teamPreviewName}>{m.name}</Text>
                      <Text style={s.teamPreviewRole}>{m.role}</Text>
                    </View>
                  </View>
                ))}
              </View>
...
const AdminCoopScreen = ({ navigation }) => {
  ...
  const {
    ...
    coops: [], loading: false,
    createCoop: null, updateCoop: null, deleteCoop: null,
  } : useCoops();

  // ── Source de données unifiée ──────────
  // En mode API, on adapte le format MongoDB → CoopCard
  ...
  const getAssignedTeam = (id) => team.filter((m) => m.assignedCoops?.includes(id));

  const handleCoopSelect = async (coop) => {
    await setSelectedCoop({
      id: coop.id, name: coop.nom, sector: coop.secteur,
      status:      coop.status === 'actif' ? 'healthy' : coop.status === 'alerte' ? 'warning' : 'critical',
      population:  parseInt(coop.population?.replace(/\s/g, '') || '0'),
      mortality:   coop.status === 'alerte' ? 0.14 : 0.02,
      temperature: parseFloat(coop.temp) || 24,
      humidity:    parseInt(coop.humidite) || 60,
    });
    navigation.navigate('CoopDashboard');
  };

  // ─────────────────────────────────────────
  // ⚡ HANDLERS — branchés mock ou API
  // ─────────────────────────────────────────
  const handleSave = USE_MOCK
    ? (data) => {
        const coopData = {
          id:           editingCoop?.id || String(Date.now()),
          nom:          data.nom,
          secteur:      data.secteur,
          status:       editingCoop?.status || 'actif',
          temp:         editingCoop?.temp   || '24.0°C',
          humidite:     editingCoop?.humidite || '60%',
          population:   editingCoop?.population || '0',
          updated:      'À l\'instant',
          espConnected: editingCoop?.espConnected ?? false,
          image:        data.image || null,
          equipements:  data.equipements,
        };
        setMockCoops((prev) => {
          const exists = prev.find((c) => c.id === coopData.id);
          if (exists) return prev.map((c) => c.id === coopData.id ? coopData : c);
          return [coopData, ...prev];
        });
        setEditingCoop(null);
        setShowModal(false);
      }
    : async (data) => {
        const payload = {
          name:        data.nom,
          sector:      data.secteur,
          population:  parseInt(data.population?.replace(/\s/g, '')) || 0,
          image:       data.image,
          equipements: data.equipements,
          status:      data.status === 'actif' ? 'healthy'
                       : data.status === 'alerte' ? 'warning' : 'critical',
        };

        const isEdit = !!editingCoop;
        const result = isEdit
          ? await updateCoop(editingCoop.id, payload)
          : await createCoop(payload);
        ...
        // Afficher le token ESP32 après création
        if (!isEdit && result.data?.esp32Token) {
          Alert.alert(
            '🔑 Token ESP32',
            `Copiez ce token dans votre code Arduino :\n\n${result.data.esp32Token}\n\nIl ne sera plus affiché.`,
            [{ text: 'Compris' }]
          );
        }
  ...
  const handleDelete = USE_MOCK
    ? (coop) => {
        Alert.alert(
          ...
          `Êtes-vous sûr de vouloir supprimer "${coop.nom}" ?`,
          [
            { text: 'Annuler', style: 'cancel' },
            { text: 'Supprimer', style: 'destructive',
              onPress: () => setMockCoops((prev) => prev.filter((c) => c.id !== coop.id)) },
          ]
        );
      }
    : (coop) => {
      ...
      <View style={s.header}>
        ...
        <View style={s.headerRight}>
          <TouchableOpacity
            ...
            onPress={() => setShowAlertPopup(true)}
            activeOpacity={0.8}
          >
      ...
      <ScrollView style={s.scroll} contentContainerStyle={s.scrollContent} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
        ...
        {filteredCoops.length === 0 ? (
          <View style={s.emptyState}>
            ...
            <TouchableOpacity style={s.emptyResetBtn} onPress={() => setSearch('')}>
              <Text style={s.emptyResetText}>Effacer</Text>
            </TouchableOpacity>
          </View>
        ) : (
...
const s = StyleSheet.create({
  ...
  popup: { backgroundColor: '#FFFFFF', borderRadius: 20, width: 300, maxHeight: 400, shadowColor: '#000', shadowOffset: { width: 0, height: 8 }, shadowOpacity: 0.15, shadowRadius: 24, elevation: 12, overflow: 'hidden' },
  ...
  popupTitle:  { fontWeight: '800', fontSize: 15, color: '#1B4332' },
  ...
  popupAlertItem: { flexDirection: 'row', alignItems: 'flex-start', gap: 10, padding: 14, borderBottomWidth: 1, borderBottomColor: '#F9FAFB' },
  popupAlertDot:  { width: 8, height: 8, borderRadius: 4, marginTop: 4, flexShrink: 0 },
  popupAlertTitle:{ fontSize: 13, fontWeight: '700', color: '#1B4332', lineHeight: 18 },
