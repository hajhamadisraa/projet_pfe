      ...
      <View style={styles.userMenu}>
        <View style={styles.userMenuHeader}>
          ...
          <View style={styles.userMenuInfo}>
            ...
            <Text style={styles.userMenuRole} numberOfLines={1}>
              {user?.role || ''}
            </Text>
          </View>
        </View>
        ...
        <TouchableOpacity style={styles.userMenuItem} onPress={onProfile} activeOpacity={0.8}>
        ...
        <TouchableOpacity style={styles.userMenuItem} onPress={onLogout} activeOpacity={0.8}>
...
const CoopListScreen = ({ navigation }) => {
  // ✅ Les coops viennent déjà filtrés depuis l'API (/coops/my)
  const coops        = useAppStore((s) => s.coops);
  const coopsLoading = useAppStore((s) => s.coopsLoading);
  const user         = useAppStore((s) => s.user);
  const unreadCount  = useAppStore((s) => s.unreadAlertsCount);
  const alerts       = useAppStore((s) => s.alerts);
  const { fetchCoops, setSelectedCoop, logout } = useAppStore();
  ...
  // ✅ Charge les coops de l'éleveur au montage
  useEffect(() => {
    fetchCoops();
  }, []);
    ...
    return (
      <SafeAreaView style={styles.safe} edges={['top']}>
        <View style={styles.loadingContainer}>
          ...
          <Text style={styles.loadingText}>Chargement de vos poulaillers...</Text>
        </View>
      </SafeAreaView>
    );
  }
                  ...
                  <View key={index} style={styles.alertItem}>
                    <View style={[
                      ...
                      <MaterialIcons
                        ...
                        size={18}
                        color={alert.severity === 'critical' ? COLORS.error : COLORS.secondary}
                      />
                    </View>
          ...
          <View style={styles.emptyState}>
            ...
            <Text style={styles.emptySubtitle}>
              {search || filter !== 'all'
                ...
                : 'Aucun poulailler ne vous a encore été affecté.'}
            </Text>
            {(search || filter !== 'all') && (
              <TouchableOpacity
